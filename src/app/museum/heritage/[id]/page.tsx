...
export default function HeritageDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
      ...
      <div className="relative w-full aspect-[16/9] bg-secondary overflow-hidden">
        ...
        {!heroFailed ? (
          ...
          <img
            ...
            className={`w-full h-full object-cover ${!isUnlocked ? "opacity-40" : ""}`}
          />
        ) : (
            ...
            {node.items.map((item) => {
              ...
              return (
                <div key={item.id} className="space-y-1">
                  <div className={`aspect-square rounded-xl overflow-hidden bg-secondary ${!collected ? "opacity-40" : ""}`}>
                    {/* eslint-disable-next-line @next/next/no-img-element */}
